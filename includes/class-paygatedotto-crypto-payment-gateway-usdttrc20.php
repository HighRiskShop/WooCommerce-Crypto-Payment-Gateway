...
class PayGateDotTo_Crypto_Payment_Gateway_Usdttrc20 extends WC_Payment_Gateway {

protected $usdttrc20_wallet_address;
protected $usdttrc20_blockchain_fees;
protected $icon_url;
...
$paygatedottocryptogateway_usdttrc20_response_minimum = wp_remote_get('https://api.paygate.to/crypto/trc20/usdt/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_usdttrc20_response_minimum)) {
    ...
    $paygatedottocryptogateway_usdttrc20_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_usdttrc20_response_minimum);
    $paygatedottocryptogateway_usdttrc20_conversion_resp_minimum = json_decode($paygatedottocryptogateway_usdttrc20_body_minimum, true);
    if ($paygatedottocryptogateway_usdttrc20_conversion_resp_minimum && isset($paygatedottocryptogateway_usdttrc20_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_usdttrc20_final_minimum = sanitize_text_field($paygatedottocryptogateway_usdttrc20_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_usdttrc20_payin_total < $paygatedottocryptogateway_usdttrc20_final_minimum) {
...
if (is_wp_error($paygatedottocryptogateway_usdttrc20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_usdttrc20_wallet_decbody && isset($paygatedottocryptogateway_usdttrc20_wallet_decbody['address_in'])) {
        ...
        $paygatedottocryptogateway_usdttrc20_gen_ipntoken = wp_kses_post($paygatedottocryptogateway_usdttrc20_wallet_decbody['ipn_token']);
...
if ($paygatedottocryptogateway_usdttrc20_genqrcode_conversion_resp && isset($paygatedottocryptogateway_usdttrc20_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->add_meta_data('paygatedotto_usdttrc20_ipntoken', $paygatedottocryptogateway_usdttrc20_gen_ipntoken, true);
...
function paygatedotto_add_instant_payment_gateway_usdttrc20($gateways) {
    $gateways[] = 'PayGateDotTo_Crypto_Payment_Gateway_Usdttrc20';
    return $gateways;
}
