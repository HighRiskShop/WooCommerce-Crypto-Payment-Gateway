...
class HighRiskShop_Crypto_Payment_Gateway_Usdcbase extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'highriskshop-crypto-payment-gateway-usdcbase';
    ...
    return false;
        ...
        $highriskshopcryptogateway_usdcbase_admin_wallet_address = isset($_POST[$this->plugin_id . $this->id . '_usdcbase_wallet_address']) ? sanitize_text_field( wp_unslash( $_POST[$this->plugin_id . $this->id . '_usdcbase_wallet_address'])) : '';

        // Check if wallet address is empty
        if (empty($highriskshopcryptogateway_usdcbase_admin_wallet_address)) {
...
$highriskshopcryptogateway_usdcbase_response = wp_remote_get('https://api.highriskshop.com/crypto/base/usdc/convert.php?value=' . $highriskshopcryptogateway_usdcbase_total . '&from=' . strtolower($highriskshopcryptogateway_usdcbase_currency), array('timeout' => 30));
...
if ($highriskshopcryptogateway_usdcbase_conversion_resp && isset($highriskshopcryptogateway_usdcbase_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$highriskshopcryptogateway_usdcbase_feesest_response = wp_remote_get('https://api.highriskshop.com/crypto/base/usdc/fees.php', array('timeout' => 30));
...
$highriskshopcryptogateway_usdcbase_revfeesest_response = wp_remote_get('https://api.highriskshop.com/crypto/base/usdc/convert.php?value=' . $highriskshopcryptogateway_usdcbase_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$highriskshopcryptogateway_usdcbase_gen_wallet = wp_remote_get('https://api.highriskshop.com/crypto/base/usdc/wallet.php?address=' . $this->usdcbase_wallet_address .'&callback=' . urlencode($highriskshopcryptogateway_usdcbase_callback), array('timeout' => 30));
...
if (is_wp_error($highriskshopcryptogateway_usdcbase_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($highriskshopcryptogateway_usdcbase_wallet_decbody && isset($highriskshopcryptogateway_usdcbase_wallet_decbody['address_in'])) {
		...
		$highriskshopcryptogateway_usdcbase_genqrcode_response = wp_remote_get('https://api.highriskshop.com/crypto/base/usdc/qrcode.php?address=' . $highriskshopcryptogateway_usdcbase_gen_addressIn, array('timeout' => 30));
...
public function before_thankyou_page($order_id) {
    ...
    echo '<p style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">' . sprintf( esc_html__('Please send %1$s %2$s to the following address:', 'crypto-payment-gateway'), '<br><strong>' . esc_html($highriskshopgateway_crypto_total) . '</strong>', esc_html__('base/usdc', 'crypto-payment-gateway') ) . '</p>';
  ...
  // Enqueue jQuery and the external script
    ...
    wp_enqueue_script('highriskshopcryptogateway-check-status', plugin_dir_url(__DIR__) . 'assets/js/highriskshopcryptogateway-payment-status-check.js?order_id=' . esc_attr($order_id) . '&nonce=' . esc_attr($highriskshopgateway_crypto_qrcode_status_nonce) . '&tickerstring=usdcbase', array('jquery'), '1.0.0', true);
...
function highriskshopcryptogateway_usdcbase_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'highriskshop-crypto-payment-gateway-usdcbase'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'highriskshop-crypto-payment-gateway-usdcbase' === $order->get_payment_method() ) {
...
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s base_usdc. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcbasepaid_value_coin, $highriskshopcryptogateway_usdcbase_paid_coin_name, $highriskshopcryptogateway_usdcbaseexpected_amount, $highriskshopcryptogateway_usdcbase_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s base_usdc. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcbasepaid_value_coin, $highriskshopcryptogateway_usdcbase_paid_coin_name, $highriskshopcryptogateway_usdcbaseexpected_amount, $highriskshopcryptogateway_usdcbase_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcbasepaid_value_coin, $highriskshopcryptogateway_usdcbase_paid_coin_name, $highriskshopcryptogateway_usdcbase_paid_txid_in));

// Return success response
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Transaction ID */
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcbasepaid_value_coin, $highriskshopcryptogateway_usdcbase_paid_coin_name, $highriskshopcryptogateway_usdcbase_paid_txid_in));
