...
$paygatedottocryptogateway_btctrc20_response = wp_remote_get('https://api.paygate.to/crypto/trc20/btc/convert.php?value=' . $paygatedottocryptogateway_btctrc20_total . '&from=' . strtolower($paygatedottocryptogateway_btctrc20_currency), array('timeout' => 30));
...
if ($paygatedottocryptogateway_btctrc20_conversion_resp && isset($paygatedottocryptogateway_btctrc20_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$paygatedottocryptogateway_btctrc20_feesest_response = wp_remote_get('https://api.paygate.to/crypto/trc20/btc/fees.php', array('timeout' => 30));
...
$paygatedottocryptogateway_btctrc20_revfeesest_response = wp_remote_get('https://api.paygate.to/crypto/trc20/btc/convert.php?value=' . $paygatedottocryptogateway_btctrc20_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$paygatedottocryptogateway_btctrc20_gen_wallet = wp_remote_get('https://api.paygate.to/crypto/trc20/btc/wallet.php?address=' . $this->btctrc20_wallet_address .'&callback=' . urlencode($paygatedottocryptogateway_btctrc20_callback), array('timeout' => 30));
...
if (is_wp_error($paygatedottocryptogateway_btctrc20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_btctrc20_wallet_decbody && isset($paygatedottocryptogateway_btctrc20_wallet_decbody['address_in'])) {
		...
		$paygatedottocryptogateway_btctrc20_genqrcode_response = wp_remote_get('https://api.paygate.to/crypto/trc20/btc/qrcode.php?address=' . $paygatedottocryptogateway_btctrc20_gen_addressIn, array('timeout' => 30));
...
function paygatedotto_add_instant_payment_gateway_btctrc20($gateways) {
    $gateways[] = 'HighRiskShop_Crypto_Payment_Gateway_Btctrc20';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'paygatedotto_add_instant_payment_gateway_btctrc20');
}
...
function paygatedottocryptogateway_btctrc20_change_order_status_callback( $request ) {
	...
	$paygatedottocryptogateway_btctrc20expected_amount = $order->get_meta('paygatedotto_btctrc20_payin_amount', true);
	$paygatedottocryptogateway_btctrc20expected_coin = $order->get_meta('paygatedotto_btctrc20_payin_amount', true);
...
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s trc20_btc. TXID: %4$s', 'crypto-payment-gateway' ), $paygatedottocryptogateway_btctrc20paid_value_coin, $paygatedottocryptogateway_btctrc20_paid_coin_name, $paygatedottocryptogateway_btctrc20expected_amount, $paygatedottocryptogateway_btctrc20_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $paygatedottocryptogateway_btctrc20paid_value_coin, $paygatedottocryptogateway_btctrc20_paid_coin_name, $paygatedottocryptogateway_btctrc20_paid_txid_in));
