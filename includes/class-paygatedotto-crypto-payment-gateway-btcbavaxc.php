...
class PayGateDotTo_Crypto_Payment_Gateway_Btcbavaxc extends WC_Payment_Gateway {
...
$paygatedottocryptogateway_btcbavaxc_response_minimum = wp_remote_get('https://api.paygate.to/crypto/avax-c/btc.b/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_btcbavaxc_response_minimum)) {
    ...
    $paygatedottocryptogateway_btcbavaxc_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_btcbavaxc_response_minimum);
    $paygatedottocryptogateway_btcbavaxc_conversion_resp_minimum = json_decode($paygatedottocryptogateway_btcbavaxc_body_minimum, true);
    if ($paygatedottocryptogateway_btcbavaxc_conversion_resp_minimum && isset($paygatedottocryptogateway_btcbavaxc_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_btcbavaxc_final_minimum = sanitize_text_field($paygatedottocryptogateway_btcbavaxc_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_btcbavaxc_payin_total < $paygatedottocryptogateway_btcbavaxc_final_minimum) {
...
if (is_wp_error($paygatedottocryptogateway_btcbavaxc_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_btcbavaxc_wallet_decbody && isset($paygatedottocryptogateway_btcbavaxc_wallet_decbody['address_in'])) {
        ...
        $paygatedottocryptogateway_btcbavaxc_gen_ipntoken = wp_kses_post($paygatedottocryptogateway_btcbavaxc_wallet_decbody['ipn_token']);
...
function paygatedotto_add_instant_payment_gateway_btcbavaxc($gateways) {
    $gateways[] = 'PayGateDotTo_Crypto_Payment_Gateway_Btcbavaxc';
    return $gateways;
}
