...
class PayGateDotTo_Crypto_Payment_Gateway_Wethpolygon extends WC_Payment_Gateway {

protected $wethpolygon_wallet_address;
protected $wethpolygon_blockchain_fees;
protected $icon_url;
...
$paygatedottocryptogateway_wethpolygon_response_minimum = wp_remote_get('https://api.paygate.to/crypto/polygon/weth/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_wethpolygon_response_minimum)) {
    ...
    $paygatedottocryptogateway_wethpolygon_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_wethpolygon_response_minimum);
    $paygatedottocryptogateway_wethpolygon_conversion_resp_minimum = json_decode($paygatedottocryptogateway_wethpolygon_body_minimum, true);
    if ($paygatedottocryptogateway_wethpolygon_conversion_resp_minimum && isset($paygatedottocryptogateway_wethpolygon_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_wethpolygon_final_minimum = sanitize_text_field($paygatedottocryptogateway_wethpolygon_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_wethpolygon_payin_total < $paygatedottocryptogateway_wethpolygon_final_minimum) {
...
if (is_wp_error($paygatedottocryptogateway_wethpolygon_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_wethpolygon_wallet_decbody && isset($paygatedottocryptogateway_wethpolygon_wallet_decbody['address_in'])) {
        ...
        $paygatedottocryptogateway_wethpolygon_gen_ipntoken = wp_kses_post($paygatedottocryptogateway_wethpolygon_wallet_decbody['ipn_token']);
...
if ($paygatedottocryptogateway_wethpolygon_genqrcode_conversion_resp && isset($paygatedottocryptogateway_wethpolygon_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->add_meta_data('paygatedotto_wethpolygon_ipntoken', $paygatedottocryptogateway_wethpolygon_gen_ipntoken, true);
...
function paygatedotto_add_instant_payment_gateway_wethpolygon($gateways) {
    $gateways[] = 'PayGateDotTo_Crypto_Payment_Gateway_Wethpolygon';
    return $gateways;
}
