...
class PayGateDotTo_Crypto_Payment_Gateway_Usdcavaxc extends WC_Payment_Gateway {

protected $usdcavaxc_wallet_address;
protected $usdcavaxc_blockchain_fees;
protected $icon_url;
...
$paygatedottocryptogateway_usdcavaxc_response_minimum = wp_remote_get('https://api.paygate.to/crypto/avax-c/usdc/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_usdcavaxc_response_minimum)) {
    ...
    $paygatedottocryptogateway_usdcavaxc_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_usdcavaxc_response_minimum);
    $paygatedottocryptogateway_usdcavaxc_conversion_resp_minimum = json_decode($paygatedottocryptogateway_usdcavaxc_body_minimum, true);
    if ($paygatedottocryptogateway_usdcavaxc_conversion_resp_minimum && isset($paygatedottocryptogateway_usdcavaxc_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_usdcavaxc_final_minimum = sanitize_text_field($paygatedottocryptogateway_usdcavaxc_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_usdcavaxc_payin_total < $paygatedottocryptogateway_usdcavaxc_final_minimum) {
...
function paygatedotto_add_instant_payment_gateway_usdcavaxc($gateways) {
    $gateways[] = 'PayGateDotTo_Crypto_Payment_Gateway_Usdcavaxc';
    return $gateways;
}
