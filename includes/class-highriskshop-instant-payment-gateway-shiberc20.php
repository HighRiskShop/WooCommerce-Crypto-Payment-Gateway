...
class HighRiskShop_Instant_Payment_Gateway_Shiberc20 extends WC_Payment_Gateway {

    public function __construct() {
        $this->id                 = 'highriskshop-instant-payment-gateway-shiberc20';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/shiberc20.png');
        $this->method_title       = esc_html__('SHIBA INU erc20 Crypto Payment Gateway With Instant Payouts', 'highriskshopcryptogateway'); // Escaping title
        $this->method_description = esc_html__('SHIBA INU erc20 Crypto Payment Gateway With Instant Payouts to your erc20_shib wallet. Allows you to accept crypto erc20/shib payments without sign up and without KYC.', 'highriskshopcryptogateway'); // Escaping description
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => esc_html__('Enable erc20_shib payment gateway', 'highriskshopcryptogateway'), // Escaping label
                'default' => 'no',
            ),
            'title' => array(
                ...
                'default'     => esc_html__('SHIBA INU erc20', 'highriskshopcryptogateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Pay via crypto SHIBA INU erc20 erc20_shib', 'highriskshopcryptogateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'shiberc20_wallet_address' => array(
                'title'       => esc_html__('Wallet Address', 'highriskshopcryptogateway'), // Escaping title
                'type'        => 'text',
                'description' => esc_html__('Insert your erc20/shib wallet address to receive instant payouts.', 'highriskshopcryptogateway'), // Escaping description
                'desc_tip'    => true,
            ),
...
$highriskshopcryptogateway_shiberc20_response = wp_remote_get('https://api.highriskshop.com/crypto/erc20/shib/convert.php?value=' . $highriskshopcryptogateway_shiberc20_total . '&from=' . strtolower($highriskshopcryptogateway_shiberc20_currency));

if (is_wp_error($highriskshopcryptogateway_shiberc20_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed due to failed currency conversion process, please try again', 'hrsshiberc20'), 'error');
    return null;
} else {
...
if ($highriskshopcryptogateway_shiberc20_conversion_resp && isset($highriskshopcryptogateway_shiberc20_conversion_resp['value_coin'])) {
    ...
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed, please try again (unsupported store currency)', 'hrsshiberc20'), 'error');
    return null;
}	
		}
		...
		$highriskshopcryptogateway_shiberc20_feesest_response = wp_remote_get('https://api.highriskshop.com/crypto/erc20/shib/fees.php');

if (is_wp_error($highriskshopcryptogateway_shiberc20_feesest_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Failed to get estimated fees, please try again', 'hrsshiberc20'), 'error');
    return null;
} else {
...
$highriskshopcryptogateway_shiberc20_revfeesest_response = wp_remote_get('https://api.highriskshop.com/crypto/erc20/shib/convert.php?value=' . $highriskshopcryptogateway_shiberc20_feesest_reference_total . '&from=usd');
...
$highriskshopcryptogateway_shiberc20_gen_wallet = wp_remote_get('https://api.highriskshop.com/crypto/erc20/shib/wallet.php?address=' . $this->shiberc20_wallet_address .'&callback=' . urlencode($highriskshopcryptogateway_shiberc20_callback));

if (is_wp_error($highriskshopcryptogateway_shiberc20_gen_wallet)) {
    // Handle error
    wc_add_notice(__('Wallet error:', 'woocommerce') . __('Payment could not be processed due to incorrect payout wallet settings, please contact website admin', 'hrsshiberc20'), 'error');
    return null;
} else {
 ...
 // Check if decoding was successful
    if ($highriskshopcryptogateway_shiberc20_wallet_decbody && isset($highriskshopcryptogateway_shiberc20_wallet_decbody['address_in'])) {
		...
		$highriskshopcryptogateway_shiberc20_genqrcode_response = wp_remote_get('https://api.highriskshop.com/crypto/erc20/shib/qrcode.php?address=' . $highriskshopcryptogateway_shiberc20_gen_addressIn);

if (is_wp_error($highriskshopcryptogateway_shiberc20_genqrcode_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Unable to generate QR code', 'hrsshiberc20'), 'error');
    return null;
} else {
...
if ($highriskshopcryptogateway_shiberc20_genqrcode_conversion_resp && isset($highriskshopcryptogateway_shiberc20_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->save();
        ...
        wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed, please try again (wallet address error)', 'shiberc20'), 'error');
...
public function before_thankyou_page($order_id) {
    ...
    echo '<div id="highriskshopcryptogateway-wrapper"><h1 style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">'
        . esc_html__('Please Complete Your Payment', 'highriskshop-instant-payment-gateway-shiberc20') 
        . '</h1>';
    ...
    echo '<p style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">' . sprintf( esc_html__('Please send %1$s %2$s to the following address:', 'highriskshop-instant-payment-gateway-shiberc20'), '<br><strong>' . esc_html($highriskshopgateway_crypto_total) . '</strong>', esc_html__('erc20/shib', 'highriskshop-instant-payment-gateway-shiberc20') ) . '</p>';
	...
	echo '<div class="' . esc_attr('highriskshopcryptogateway-unpaid') . '" id="' . esc_attr('highriskshop-payment-status-message') . '" style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">'
                . esc_html__('Waiting for payment', 'highriskshop-instant-payment-gateway-shiberc20')
                . '</div><br><hr><br>';	
 ...
 ?>
            ...
            jQuery(document).ready(function($) {
                function highriskshopcryptogateway_payment_status() {
                    $.ajax({
                        url: '<?php echo esc_url(rest_url('highriskshopcryptogateway/v1/highriskshopcryptogateway-check-order-status-shiberc20/')); ?>',
                        method: 'GET',
                        data: {
                        ...
                        success: function(response) {
                            if (response.status === 'processing' || response.status === 'completed') {
                                $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Payment received', 'highriskshop-instant-payment-gateway-shiberc20')); ?>')
							    ...
							    $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Payment failed, you may have sent incorrect amount or token. Contact support', 'highriskshop-instant-payment-gateway-shiberc20')); ?>')
								.removeClass('highriskshopcryptogateway-unpaid')
								.addClass('<?php echo esc_js(esc_attr('highriskshopcryptogateway-failed')); ?>');
								$('#highriskshopcryptogateway-wrapper').remove();
							} else {
                                $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Waiting for payment', 'highriskshop-instant-payment-gateway-shiberc20')); ?>');
                            }
                        },
                        error: function() {
                            $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Error checking payment status. Please refresh the page.', 'highriskshop-instant-payment-gateway-shiberc20')); ?>');
                        }
                    });
                }
...
function highriskshop_add_instant_payment_gateway_shiberc20($gateways) {
    $gateways[] = 'HighRiskShop_Instant_Payment_Gateway_Shiberc20';
    return $gateways;
}
...
// Add custom endpoint for reading crypto payment status
   ...
   function highriskshopcryptogateway_shiberc20_check_order_status_rest_endpoint() {
    ...
    function highriskshopcryptogateway_shiberc20_check_order_status_callback($request) {
        ...
        if (empty($order_id)) {
            return new WP_Error('missing_order_id', __('Order ID parameter is missing.', 'highriskshop-instant-payment-gateway-shiberc20'), array('status' => 400));
        }

        $order = wc_get_order($order_id);

        if (!$order) {
            return new WP_Error('invalid_order', __('Invalid order ID.', 'highriskshop-instant-payment-gateway-shiberc20'), array('status' => 404));
        }
        ...
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'highriskshop-instant-payment-gateway-shiberc20' ), array( 'status' => 403 ) );
    }
...
function highriskshopcryptogateway_shiberc20_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'highriskshop-instant-payment-gateway-shiberc20'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'highriskshop-instant-payment-gateway-shiberc20' === $order->get_payment_method() ) {
...
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s erc20_shib. TXID: %4$s', 'highriskshop-instant-payment-gateway-shiberc20' ), $highriskshopcryptogateway_shiberc20paid_value_coin, $highriskshopcryptogateway_shiberc20_paid_coin_name, $highriskshopcryptogateway_shiberc20expected_amount, $highriskshopcryptogateway_shiberc20_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s erc20_shib. TXID: %4$s', 'highriskshop-instant-payment-gateway-shiberc20' ), $highriskshopcryptogateway_shiberc20paid_value_coin, $highriskshopcryptogateway_shiberc20_paid_coin_name, $highriskshopcryptogateway_shiberc20expected_amount, $highriskshopcryptogateway_shiberc20_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'highriskshop-instant-payment-gateway-shiberc20' ), $highriskshopcryptogateway_shiberc20paid_value_coin, $highriskshopcryptogateway_shiberc20_paid_coin_name, $highriskshopcryptogateway_shiberc20_paid_txid_in));

// Return success response
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Transaction ID */
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'highriskshop-instant-payment-gateway-shiberc20' ), $highriskshopcryptogateway_shiberc20paid_value_coin, $highriskshopcryptogateway_shiberc20_paid_coin_name, $highriskshopcryptogateway_shiberc20_paid_txid_in));
        return array( 'message' => 'Order status changed to processing.' );
		}
    } else {
        // Return error response if conditions are not met
        return new WP_Error( 'order_not_eligible', __( 'Order is not eligible for status change.', 'highriskshop-instant-payment-gateway-shiberc20' ), array( 'status' => 400 ) );
    }
}
?>