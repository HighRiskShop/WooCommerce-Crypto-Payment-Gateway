...
class HighRiskShop_Crypto_Payment_Gateway_Ethoptimism extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'highriskshop-crypto-payment-gateway-ethoptimism';
    ...
    return false;
        ...
        $highriskshopcryptogateway_ethoptimism_admin_wallet_address = isset($_POST[$this->plugin_id . $this->id . '_ethoptimism_wallet_address']) ? sanitize_text_field( wp_unslash( $_POST[$this->plugin_id . $this->id . '_ethoptimism_wallet_address'])) : '';

        // Check if wallet address is empty
        if (empty($highriskshopcryptogateway_ethoptimism_admin_wallet_address)) {
...
public function before_thankyou_page($order_id) {
    ...
    echo '<p style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">' . sprintf( esc_html__('Please send %1$s %2$s to the following address:', 'crypto-payment-gateway'), '<br><strong>' . esc_html($highriskshopgateway_crypto_total) . '</strong>', esc_html__('optimism/eth', 'crypto-payment-gateway') ) . '</p>';
  ...
  // Enqueue jQuery and the external script
    ...
    wp_enqueue_script('highriskshopcryptogateway-check-status', plugin_dir_url(__DIR__) . 'assets/js/highriskshopcryptogateway-payment-status-check.js?order_id=' . esc_attr($order_id) . '&nonce=' . esc_attr($highriskshopgateway_crypto_qrcode_status_nonce) . '&tickerstring=ethoptimism', array('jquery'), '1.0.0', true);
...
function highriskshopcryptogateway_ethoptimism_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'highriskshop-crypto-payment-gateway-ethoptimism'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'highriskshop-crypto-payment-gateway-ethoptimism' === $order->get_payment_method() ) {
...
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s optimism_eth. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_ethoptimismpaid_value_coin, $highriskshopcryptogateway_ethoptimism_paid_coin_name, $highriskshopcryptogateway_ethoptimismexpected_amount, $highriskshopcryptogateway_ethoptimism_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s optimism_eth. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_ethoptimismpaid_value_coin, $highriskshopcryptogateway_ethoptimism_paid_coin_name, $highriskshopcryptogateway_ethoptimismexpected_amount, $highriskshopcryptogateway_ethoptimism_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_ethoptimismpaid_value_coin, $highriskshopcryptogateway_ethoptimism_paid_coin_name, $highriskshopcryptogateway_ethoptimism_paid_txid_in));

// Return success response
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Transaction ID */
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_ethoptimismpaid_value_coin, $highriskshopcryptogateway_ethoptimism_paid_coin_name, $highriskshopcryptogateway_ethoptimism_paid_txid_in));
