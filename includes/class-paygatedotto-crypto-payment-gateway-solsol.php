...
class PayGateDotTo_Crypto_Payment_Gateway_Solsol extends WC_Payment_Gateway {

protected $solsol_wallet_address;
protected $solsol_blockchain_fees;
protected $icon_url;
...
$paygatedottocryptogateway_solsol_response_minimum = wp_remote_get('https://api.paygate.to/crypto/sol/sol/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_solsol_response_minimum)) {
    ...
    $paygatedottocryptogateway_solsol_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_solsol_response_minimum);
    $paygatedottocryptogateway_solsol_conversion_resp_minimum = json_decode($paygatedottocryptogateway_solsol_body_minimum, true);
    if ($paygatedottocryptogateway_solsol_conversion_resp_minimum && isset($paygatedottocryptogateway_solsol_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_solsol_final_minimum = sanitize_text_field($paygatedottocryptogateway_solsol_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_solsol_payin_total < $paygatedottocryptogateway_solsol_final_minimum) {
...
if (is_wp_error($paygatedottocryptogateway_solsol_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_solsol_wallet_decbody && isset($paygatedottocryptogateway_solsol_wallet_decbody['address_in'])) {
        ...
        $paygatedottocryptogateway_solsol_gen_ipntoken = wp_kses_post($paygatedottocryptogateway_solsol_wallet_decbody['ipn_token']);
...
if ($paygatedottocryptogateway_solsol_genqrcode_conversion_resp && isset($paygatedottocryptogateway_solsol_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->add_meta_data('paygatedotto_solsol_ipntoken', $paygatedottocryptogateway_solsol_gen_ipntoken, true);
...
function paygatedotto_add_instant_payment_gateway_solsol($gateways) {
    $gateways[] = 'PayGateDotTo_Crypto_Payment_Gateway_Solsol';
    return $gateways;
}
