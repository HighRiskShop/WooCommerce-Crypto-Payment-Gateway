...
class HighRiskShop_Crypto_Payment_Gateway_Usdcpolygon extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'highriskshop-crypto-payment-gateway-usdcpolygon';
    ...
    return false;
        ...
        $highriskshopcryptogateway_usdcpolygon_admin_wallet_address = isset($_POST[$this->plugin_id . $this->id . '_usdcpolygon_wallet_address']) ? sanitize_text_field( wp_unslash( $_POST[$this->plugin_id . $this->id . '_usdcpolygon_wallet_address'])) : '';

        // Check if wallet address is empty
        if (empty($highriskshopcryptogateway_usdcpolygon_admin_wallet_address)) {
...
$highriskshopcryptogateway_usdcpolygon_response = wp_remote_get('https://api.highriskshop.com/crypto/polygon/usdc/convert.php?value=' . $highriskshopcryptogateway_usdcpolygon_total . '&from=' . strtolower($highriskshopcryptogateway_usdcpolygon_currency), array('timeout' => 30));
...
if ($highriskshopcryptogateway_usdcpolygon_conversion_resp && isset($highriskshopcryptogateway_usdcpolygon_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$highriskshopcryptogateway_usdcpolygon_feesest_response = wp_remote_get('https://api.highriskshop.com/crypto/polygon/usdc/fees.php', array('timeout' => 30));
...
$highriskshopcryptogateway_usdcpolygon_revfeesest_response = wp_remote_get('https://api.highriskshop.com/crypto/polygon/usdc/convert.php?value=' . $highriskshopcryptogateway_usdcpolygon_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$highriskshopcryptogateway_usdcpolygon_gen_wallet = wp_remote_get('https://api.highriskshop.com/crypto/polygon/usdc/wallet.php?address=' . $this->usdcpolygon_wallet_address .'&callback=' . urlencode($highriskshopcryptogateway_usdcpolygon_callback), array('timeout' => 30));
...
if (is_wp_error($highriskshopcryptogateway_usdcpolygon_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($highriskshopcryptogateway_usdcpolygon_wallet_decbody && isset($highriskshopcryptogateway_usdcpolygon_wallet_decbody['address_in'])) {
		...
		$highriskshopcryptogateway_usdcpolygon_genqrcode_response = wp_remote_get('https://api.highriskshop.com/crypto/polygon/usdc/qrcode.php?address=' . $highriskshopcryptogateway_usdcpolygon_gen_addressIn, array('timeout' => 30));
...
public function before_thankyou_page($order_id) {
    ...
    echo '<p style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">' . sprintf( esc_html__('Please send %1$s %2$s to the following address:', 'crypto-payment-gateway'), '<br><strong>' . esc_html($highriskshopgateway_crypto_total) . '</strong>', esc_html__('polygon/usdc', 'crypto-payment-gateway') ) . '</p>';
  ...
  // Enqueue jQuery and the external script
    ...
    wp_enqueue_script('highriskshopcryptogateway-check-status', plugin_dir_url(__DIR__) . 'assets/js/highriskshopcryptogateway-payment-status-check.js?order_id=' . esc_attr($order_id) . '&nonce=' . esc_attr($highriskshopgateway_crypto_qrcode_status_nonce) . '&tickerstring=usdcpolygon', array('jquery'), '1.0.0', true);
...
function highriskshopcryptogateway_usdcpolygon_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'highriskshop-crypto-payment-gateway-usdcpolygon'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'highriskshop-crypto-payment-gateway-usdcpolygon' === $order->get_payment_method() ) {
...
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s polygon_usdc. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcpolygonpaid_value_coin, $highriskshopcryptogateway_usdcpolygon_paid_coin_name, $highriskshopcryptogateway_usdcpolygonexpected_amount, $highriskshopcryptogateway_usdcpolygon_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s polygon_usdc. TXID: %4$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcpolygonpaid_value_coin, $highriskshopcryptogateway_usdcpolygon_paid_coin_name, $highriskshopcryptogateway_usdcpolygonexpected_amount, $highriskshopcryptogateway_usdcpolygon_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcpolygonpaid_value_coin, $highriskshopcryptogateway_usdcpolygon_paid_coin_name, $highriskshopcryptogateway_usdcpolygon_paid_txid_in));

// Return success response
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Transaction ID */
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'crypto-payment-gateway' ), $highriskshopcryptogateway_usdcpolygonpaid_value_coin, $highriskshopcryptogateway_usdcpolygon_paid_coin_name, $highriskshopcryptogateway_usdcpolygon_paid_txid_in));
