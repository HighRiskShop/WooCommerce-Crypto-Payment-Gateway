...
class HighRiskShop_Instant_Payment_Gateway_Linkoptimism extends WC_Payment_Gateway {

    public function __construct() {
        $this->id                 = 'highriskshop-instant-payment-gateway-linkoptimism';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/linkoptimism.png');
        $this->method_title       = esc_html__('ChainLink Token optimism Crypto Payment Gateway With Instant Payouts', 'highriskshopcryptogateway'); // Escaping title
        $this->method_description = esc_html__('ChainLink Token optimism Crypto Payment Gateway With Instant Payouts to your optimism_link wallet. Allows you to accept crypto optimism/link payments without sign up and without KYC.', 'highriskshopcryptogateway'); // Escaping description
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => esc_html__('Enable optimism_link payment gateway', 'highriskshopcryptogateway'), // Escaping label
                'default' => 'no',
            ),
            'title' => array(
                ...
                'default'     => esc_html__('ChainLink Token optimism', 'highriskshopcryptogateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Pay via crypto ChainLink Token optimism optimism_link', 'highriskshopcryptogateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'linkoptimism_wallet_address' => array(
                'title'       => esc_html__('Wallet Address', 'highriskshopcryptogateway'), // Escaping title
                'type'        => 'text',
                'description' => esc_html__('Insert your optimism/link wallet address to receive instant payouts.', 'highriskshopcryptogateway'), // Escaping description
                'desc_tip'    => true,
            ),
...
$highriskshopcryptogateway_linkoptimism_response = wp_remote_get('https://api.highriskshop.com/crypto/optimism/link/convert.php?value=' . $highriskshopcryptogateway_linkoptimism_total . '&from=' . strtolower($highriskshopcryptogateway_linkoptimism_currency));

if (is_wp_error($highriskshopcryptogateway_linkoptimism_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed due to failed currency conversion process, please try again', 'hrslinkoptimism'), 'error');
    return null;
} else {
...
if ($highriskshopcryptogateway_linkoptimism_conversion_resp && isset($highriskshopcryptogateway_linkoptimism_conversion_resp['value_coin'])) {
    ...
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed, please try again (unsupported store currency)', 'hrslinkoptimism'), 'error');
    return null;
}	
		}
		...
		$highriskshopcryptogateway_linkoptimism_feesest_response = wp_remote_get('https://api.highriskshop.com/crypto/optimism/link/fees.php');

if (is_wp_error($highriskshopcryptogateway_linkoptimism_feesest_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Failed to get estimated fees, please try again', 'hrslinkoptimism'), 'error');
    return null;
} else {
...
$highriskshopcryptogateway_linkoptimism_revfeesest_response = wp_remote_get('https://api.highriskshop.com/crypto/optimism/link/convert.php?value=' . $highriskshopcryptogateway_linkoptimism_feesest_reference_total . '&from=usd');
...
$highriskshopcryptogateway_linkoptimism_gen_wallet = wp_remote_get('https://api.highriskshop.com/crypto/optimism/link/wallet.php?address=' . $this->linkoptimism_wallet_address .'&callback=' . urlencode($highriskshopcryptogateway_linkoptimism_callback));

if (is_wp_error($highriskshopcryptogateway_linkoptimism_gen_wallet)) {
    // Handle error
    wc_add_notice(__('Wallet error:', 'woocommerce') . __('Payment could not be processed due to incorrect payout wallet settings, please contact website admin', 'hrslinkoptimism'), 'error');
    return null;
} else {
 ...
 // Check if decoding was successful
    if ($highriskshopcryptogateway_linkoptimism_wallet_decbody && isset($highriskshopcryptogateway_linkoptimism_wallet_decbody['address_in'])) {
		...
		$highriskshopcryptogateway_linkoptimism_genqrcode_response = wp_remote_get('https://api.highriskshop.com/crypto/optimism/link/qrcode.php?address=' . $highriskshopcryptogateway_linkoptimism_gen_addressIn);

if (is_wp_error($highriskshopcryptogateway_linkoptimism_genqrcode_response)) {
    // Handle error
    wc_add_notice(__('Payment error:', 'woocommerce') . __('Unable to generate QR code', 'hrslinkoptimism'), 'error');
    return null;
} else {
...
if ($highriskshopcryptogateway_linkoptimism_genqrcode_conversion_resp && isset($highriskshopcryptogateway_linkoptimism_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->save();
        ...
        wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed, please try again (wallet address error)', 'linkoptimism'), 'error');
...
public function before_thankyou_page($order_id) {
    ...
    echo '<div id="highriskshopcryptogateway-wrapper"><h1 style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">'
        . esc_html__('Please Complete Your Payment', 'highriskshop-instant-payment-gateway-linkoptimism') 
        . '</h1>';
    ...
    echo '<p style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">' . sprintf( esc_html__('Please send %1$s %2$s to the following address:', 'highriskshop-instant-payment-gateway-linkoptimism'), '<br><strong>' . esc_html($highriskshopgateway_crypto_total) . '</strong>', esc_html__('optimism/link', 'highriskshop-instant-payment-gateway-linkoptimism') ) . '</p>';
	...
	echo '<div class="' . esc_attr('highriskshopcryptogateway-unpaid') . '" id="' . esc_attr('highriskshop-payment-status-message') . '" style="' . esc_attr('text-align:center;max-width:100%;margin:0 auto;') . '">'
                . esc_html__('Waiting for payment', 'highriskshop-instant-payment-gateway-linkoptimism')
                . '</div><br><hr><br>';	
 ...
 ?>
            ...
            jQuery(document).ready(function($) {
                function highriskshopcryptogateway_payment_status() {
                    $.ajax({
                        url: '<?php echo esc_url(rest_url('highriskshopcryptogateway/v1/highriskshopcryptogateway-check-order-status-linkoptimism/')); ?>',
                        method: 'GET',
                        data: {
                        ...
                        success: function(response) {
                            if (response.status === 'processing' || response.status === 'completed') {
                                $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Payment received', 'highriskshop-instant-payment-gateway-linkoptimism')); ?>')
							    ...
							    $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Payment failed, you may have sent incorrect amount or token. Contact support', 'highriskshop-instant-payment-gateway-linkoptimism')); ?>')
								.removeClass('highriskshopcryptogateway-unpaid')
								.addClass('<?php echo esc_js(esc_attr('highriskshopcryptogateway-failed')); ?>');
								$('#highriskshopcryptogateway-wrapper').remove();
							} else {
                                $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Waiting for payment', 'highriskshop-instant-payment-gateway-linkoptimism')); ?>');
                            }
                        },
                        error: function() {
                            $('#highriskshop-payment-status-message').text('<?php echo esc_js(__('Error checking payment status. Please refresh the page.', 'highriskshop-instant-payment-gateway-linkoptimism')); ?>');
                        }
                    });
                }
...
function highriskshop_add_instant_payment_gateway_linkoptimism($gateways) {
    $gateways[] = 'HighRiskShop_Instant_Payment_Gateway_Linkoptimism';
    return $gateways;
}
...
// Add custom endpoint for reading crypto payment status
   ...
   function highriskshopcryptogateway_linkoptimism_check_order_status_rest_endpoint() {
    ...
    function highriskshopcryptogateway_linkoptimism_check_order_status_callback($request) {
        ...
        if (empty($order_id)) {
            return new WP_Error('missing_order_id', __('Order ID parameter is missing.', 'highriskshop-instant-payment-gateway-linkoptimism'), array('status' => 400));
        }

        $order = wc_get_order($order_id);

        if (!$order) {
            return new WP_Error('invalid_order', __('Invalid order ID.', 'highriskshop-instant-payment-gateway-linkoptimism'), array('status' => 404));
        }
        ...
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'highriskshop-instant-payment-gateway-linkoptimism' ), array( 'status' => 403 ) );
    }
...
function highriskshopcryptogateway_linkoptimism_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'highriskshop-instant-payment-gateway-linkoptimism'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'highriskshop-instant-payment-gateway-linkoptimism' === $order->get_payment_method() ) {
...
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s optimism_link. TXID: %4$s', 'highriskshop-instant-payment-gateway-linkoptimism' ), $highriskshopcryptogateway_linkoptimismpaid_value_coin, $highriskshopcryptogateway_linkoptimism_paid_coin_name, $highriskshopcryptogateway_linkoptimismexpected_amount, $highriskshopcryptogateway_linkoptimism_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
$order->add_order_note(sprintf( __( '[Order Failed] Customer sent %1$s %2$s instead of %3$s optimism_link. TXID: %4$s', 'highriskshop-instant-payment-gateway-linkoptimism' ), $highriskshopcryptogateway_linkoptimismpaid_value_coin, $highriskshopcryptogateway_linkoptimism_paid_coin_name, $highriskshopcryptogateway_linkoptimismexpected_amount, $highriskshopcryptogateway_linkoptimism_paid_txid_in));
		...
		$order->update_status('processing', sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'highriskshop-instant-payment-gateway-linkoptimism' ), $highriskshopcryptogateway_linkoptimismpaid_value_coin, $highriskshopcryptogateway_linkoptimism_paid_coin_name, $highriskshopcryptogateway_linkoptimism_paid_txid_in));

// Return success response
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Transaction ID */
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'highriskshop-instant-payment-gateway-linkoptimism' ), $highriskshopcryptogateway_linkoptimismpaid_value_coin, $highriskshopcryptogateway_linkoptimism_paid_coin_name, $highriskshopcryptogateway_linkoptimism_paid_txid_in));
        return array( 'message' => 'Order status changed to processing.' );
		}
    } else {
        // Return error response if conditions are not met
        return new WP_Error( 'order_not_eligible', __( 'Order is not eligible for status change.', 'highriskshop-instant-payment-gateway-linkoptimism' ), array( 'status' => 400 ) );
    }
}
?>