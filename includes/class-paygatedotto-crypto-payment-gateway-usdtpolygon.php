...
class PayGateDotTo_Crypto_Payment_Gateway_Usdtpolygon extends WC_Payment_Gateway {

protected $usdtpolygon_wallet_address;
protected $usdtpolygon_blockchain_fees;
protected $icon_url;
...
$paygatedottocryptogateway_usdtpolygon_response_minimum = wp_remote_get('https://api.paygate.to/crypto/polygon/usdt/info.php', array('timeout' => 30));
if (is_wp_error($paygatedottocryptogateway_usdtpolygon_response_minimum)) {
    ...
    $paygatedottocryptogateway_usdtpolygon_body_minimum = wp_remote_retrieve_body($paygatedottocryptogateway_usdtpolygon_response_minimum);
    $paygatedottocryptogateway_usdtpolygon_conversion_resp_minimum = json_decode($paygatedottocryptogateway_usdtpolygon_body_minimum, true);
    if ($paygatedottocryptogateway_usdtpolygon_conversion_resp_minimum && isset($paygatedottocryptogateway_usdtpolygon_conversion_resp_minimum['minimum'])) {
        $paygatedottocryptogateway_usdtpolygon_final_minimum = sanitize_text_field($paygatedottocryptogateway_usdtpolygon_conversion_resp_minimum['minimum']);
        if ($paygatedottocryptogateway_usdtpolygon_payin_total < $paygatedottocryptogateway_usdtpolygon_final_minimum) {
...
if (is_wp_error($paygatedottocryptogateway_usdtpolygon_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($paygatedottocryptogateway_usdtpolygon_wallet_decbody && isset($paygatedottocryptogateway_usdtpolygon_wallet_decbody['address_in'])) {
        ...
        $paygatedottocryptogateway_usdtpolygon_gen_ipntoken = wp_kses_post($paygatedottocryptogateway_usdtpolygon_wallet_decbody['ipn_token']);
...
if ($paygatedottocryptogateway_usdtpolygon_genqrcode_conversion_resp && isset($paygatedottocryptogateway_usdtpolygon_genqrcode_conversion_resp['qr_code'])) {
    ...
    $order->add_meta_data('paygatedotto_usdtpolygon_ipntoken', $paygatedottocryptogateway_usdtpolygon_gen_ipntoken, true);
